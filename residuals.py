...
meds,stds,stars = new_extract_med_std(conn,ra,dec,radius,filt_choice,tel_choice,overriding_results=np.asarray(random_integers))
...
query = 'SELECT img_id, date_obs_jd FROM images'	    
results = phot_db.query_to_astropy_table(conn, query, args=())
images = list(set(results['img_id']))
times = results['date_obs_jd']
...
for image_id,hjd in tqdm(results):
	...
	for star_id,star_mag in star_meds_table:
		query = 'SELECT filter, facility, hjd, calibrated_mag, calibrated_mag_err FROM phot WHERE star_id="'+str(star_id)+'" AND image ="' + str(image_id) + '" AND filter="'+str(filt_choice)+'" AND facility="'+str(tel_choice)+'"'	    
...
#import pdb; pdb.set_trace()
results_table = Table({'img_id': np.asarray(t['col0']), 'error': errors}, names=('img_id', 'error'))
